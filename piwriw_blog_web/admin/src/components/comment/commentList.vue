  ...
  methods: {
    ...
    commentCheck(id) {
      this.$confirm({
        ...
        onOk: async () => {
          ...
          if (res_status.data === 1) return this.$message.error('该评论已处于显示状态，无需审核')
          const { data: res } = await this.$http.put(`checkcomment/${id}/1`, )
    ...
    commentUncheck(id) {
      this.$confirm({
        ...
        onOk: async () => {
          const { data: res_status } = await this.$http.get(`comment/info/${id}`)
          if (res_status.data === 0) return this.$message.error('该评论已处于未审核状态，无需撤下')
          const { data: res } = await this.$http.put(`checkcomment/${id}/0`, )
          if (res.msg !== 'success') return this.$message.error(res.msg)
